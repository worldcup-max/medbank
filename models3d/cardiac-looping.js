 ...
 *
 * AXES. +x = embryo's LEFT, +y = CRANIAL, +z = VENTRAL. CORRECTED BY REVIEW 2026-09-10: this
 * comment previously said +x = RIGHT, which is the opposite of the corpus convention and would send
 * the next procedural model out mirrored. Measured, not assumed: six BodyParts3D right/left pairs in
 * viz-training/meshes-lite (tenth rib, hip bone, epididymis, both lung lobes, pyramidalis) all put
 * RIGHT at negative x and LEFT at positive x, and viz3d.js puts the 'anterior' camera at +z.
 * The GEOMETRY was already right: the default build swings the bulboventricular limb to -x, i.e. the
 * embryo's right, which is the D-loop; 'mirror' sends it to +x, the L-loop. Only this note was wrong.
 *
 * OPEN DEFECT, REVIEW 2026-09-10, NOT FIXED HERE — the second half of the old note claimed the limb
 * 'swings ventrally'. It does not. Measured in world space at t=1: ventricle centre z 0 -> -1.12,
 * bulbus 0 -> -0.82, atrium 0 -> -0.93. The whole tube bends DORSALLY, so the loop's convexity points
 * backwards and the atrium never comes to lie behind and above the ventricle. View 3 of the scene is
 * titled 'swings ventrally, caudally and to the RIGHT' and view 5 says the inflow end ends up 'behind
 * and above the ventricles'; the geometry contradicts both. This is a torsion problem (PSI0/TAU
 * below), not a sign flip: flipping z would send the atrium ventral, which is equally wrong. Left for
 * the build task with the scene marked changes-requested.
 *
...
/* REMOVED BY REVIEW 2026-09-10 — a DEPTH_ORDER table sat here, commented as being "used for
   polygonOffset ranking so nothing z-fights". It was never read by anything, and the claim was the
   wrong one to make: RENDER-STANDARD 2.5 says depth-rank polygonOffset does active harm on separated
   solids, which is exactly what this model is. Nothing z-fights here because the segments meet at
   their waists and overlap slightly, not because of any ranking. Deleted rather than wired up. */
...
window.MB3D_MODELS['cardiac-looping'] = {
  ...
  FULL: { endocardium: true, pericardium: true, mesocardium: true },
  /* Variants a scene may ask for as ref flags, e.g. "cardiac-looping#ventricle@1+mirror".
     mirror = the L-loop: the frame is seeded the other way, so it is a correctly wound solid and not a
     negatively scaled one. See the MIRRORING note above. */
  VARIANTS: { mirror: 'L-loop — the tube turns to the left instead of the right' }
};

/* the spike viewer's contract, kept so spike/heart.html still runs off this file if pointed at it */
