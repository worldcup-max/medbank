...
const GEN_TIMEOUT_MS = Number(process.env.GEN_TIMEOUT_MS) || 240000;            // text-model calls must never hang forever, but allow big reasoning+JSON gens (fail fast → the UI can show a real error)
...
async function buildExtra(kind, level, note, model){
  ...
  const maxTok = (row && row.max_tokens) || (kind==="qbank" ? 24000 : 8000);   // reasoning models spend ~6k just thinking before emitting JSON — give real headroom
