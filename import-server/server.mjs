...
STEP 1c — REDUNDANT / CONTRAINDICATED THERAPY CHECK (V1.8.1, MANDATORY — do this explicitly, do not skip. This check is CLASS-FIRST, INDICATION-SECOND: you MUST complete the duplication comparison BEFORE you consider whether the keyed drug has a legitimate indication. A correct indication does NOT override a duplication finding.):
(i) INVENTORY every medication the patient is CURRENTLY taking as stated in the stem.
(ii) Normalize each current medication to its THERAPEUTIC CLASS, including combination/dual-class drugs. Reference map (non-exhaustive): carvedilol -> beta-blocker (combined alpha/beta); propranolol -> beta-blocker; metoprolol/bisoprolol/nebivolol -> beta-blocker; labetalol -> beta-blocker (combined alpha/beta); empagliflozin/dapagliflozin/canagliflozin -> SGLT2 inhibitor; lisinopril/ramipril/enalapril -> ACE inhibitor; losartan/candesartan/valsartan -> ARB; sacubitril/valsartan (ARNI) -> CONTAINS an ARB component (so an additional ACE-I or ARB is duplicative); spironolactone/eplerenone -> MRA; amiloride -> potassium-sparing diuretic (overlapping hyperkalaemia risk with MRAs); metformin -> biguanide; verapamil/diltiazem -> non-dihydropyridine CCB.
(iii) Identify the THERAPEUTIC CLASS of the therapy in the keyed answer.
(iv) DUPLICATION COMPARISON (do this NOW, before any indication reasoning): if the keyed therapy is another agent of a class ALREADY PRESENT in the inventory AND concurrent use would ordinarily constitute DUPLICATE therapy rather than a deliberate, guideline-endorsed combination, then the keyed answer is REDUNDANT: set valid=false, name BOTH drugs and the shared class, and state explicitly that the appropriate action is generally to SWITCH/ADJUST the existing agent rather than ADD a second agent of the same class. Concrete duplications to catch: dapagliflozin added when already on empagliflozin (two SGLT2 inhibitors); propranolol added when already on carvedilol (two beta-blockers — switch, do not stack); a second ACE-I/ARB (including on top of an ARNI); a second MRA. CRITICAL: the keyed drug having a genuine independent indication (e.g. propranolol being appropriate for thyrotoxic adrenergic tone) does NOT rescue it — if the class is already present, ADDING another is still wrong; the answer should have been to switch.
(v) LEGITIMATE-COMBINATION GUARD (avoid false positives): do NOT flag as duplication two agents from a related therapeutic area that are intentionally and appropriately used together per guidelines (e.g. an ACE-I/ARB + an MRA + a beta-blocker + an SGLT2 inhibitor are FOUR DIFFERENT classes and are the correct combined HFrEF regimen; a loop diuretic + a thiazide for sequential nephron blockade; dual bronchodilators of different classes). Duplication means SAME class / same mechanism used redundantly, not different classes combined on purpose.
(vi) CONTRAINDICATION: also set valid=false if the keyed therapy is CONTRAINDICATED by a stated drug or comorbidity (e.g. pioglitazone or a non-dihydropyridine CCB in HFrEF; stopping metformin solely for eGFR >=30 when guidelines only dose-reduce), naming the specific conflict.
(vii) Only AFTER (iv)–(vi) do you assess the keyed therapy's positive indication.
Also sanity-check any stated electrophysiology/pharmacology mechanism wording (e.g. hyperkalaemia causes peaked T waves via ALTERED/accelerated repolarisation, not "delayed repolarisation"); set valid=false and name the wording error.
...
SINGLE-BEST-ANSWER DISCIPLINE (V1.8): the keyed answer must be UNIQUELY defensible given the EXACT facts, values and timeline YOU wrote. Before emitting, test EVERY distractor against your own stem: could a competent clinician still defend it as the "next step"/"best answer" under those specific facts? If any distractor remains genuinely defensible (e.g. two treatments both indicated, an immediate step competing with a definitive one, a diagnosis not excluded by the given data), you MUST either (a) add a fact to the stem that clearly makes the key dominate, or (b) change the keyed answer — do NOT rely on the reader overlooking the competitor. Also verify the keyed management is not REDUNDANT with or CONTRAINDICATED by the medications the patient is ALREADY ON (never add a second drug of a class they already take — e.g. a second SGLT2 inhibitor, a second ACE inhibitor). If you cannot make the key uniquely correct, return {"ok":false}.
