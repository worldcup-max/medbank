...
import { runCandidate, applyHumanReview, readinessGate, qaScore, dependencyGate, clinicalGate, sbaGate, qaVerdict, redundancyGate, rebalanceOptions, normalizeOptions, diversitySignal, sourceBudget } from "./integrated.mjs";
...
STEP 1c — CONTRAINDICATION / MECHANISM-WORDING CHECK (same-drug-class DUPLICATION is handled by a separate deterministic gate — do NOT spend effort re-deriving drug-class overlap here; concentrate on clinical reasoning): (a) valid=false if the keyed therapy is CONTRAINDICATED by a stated drug or comorbidity (e.g. pioglitazone or a non-dihydropyridine calcium-channel blocker in HFrEF; stopping metformin solely for eGFR >=30 when guidelines only dose-reduce), naming the specific conflict. (b) Sanity-check any stated electrophysiology/pharmacology mechanism WORDING (e.g. hyperkalaemia causes peaked T waves via ALTERED/accelerated repolarisation, not "delayed repolarisation"); set valid=false and name the wording error.
STEP 2 — State the answer YOUR reconstruction lands on. BEFORE returning any verdict, re-confirm you applied the STEP 1b quantitative mechanism/trajectory check: a "pathognomonic", classic, or MRI finding NEVER overrides contradictory numbers — e.g. a sodium rise of only ~2 mmol/L cannot cause osmotic demyelination no matter what an MRI is said to show. Do not return valid=true if the numbers do not support the mechanism the keyed diagnosis requires.
...
app.post("/admin/integrated/review-debug", async (req,res)=>{
  ...
  try{ if(!await requireAdmin(req)) return res.status(403).json({ error:"admins only" });
    ...
    const redundancy=redundancyGate(content);   // deterministic same-class duplicate-therapy check (V1.8.2)
    const qa=qaVerdict({ dependency:dep, integration_quality:(depRaw&&depRaw.quality)||"adequate", clinical:clin, sba, redundancy });
    res.json({ ok:true, family:proposal.integration_family, keyed_answer:content.answer,
      R1_dependency:{ raw:depRaw, gate:dep }, R2_clinical:{ raw:clinRaw, gate:clin }, R3_sba:{ raw:sbaRaw, gate:sba }, R4_redundancy:redundancy,
