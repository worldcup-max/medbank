...
STEP 1c — REDUNDANT / CONTRAINDICATED THERAPY CHECK (V1.8.1, CLASS-FIRST, INDICATION-SECOND): do the duplication comparison BEFORE judging whether the keyed drug has a legitimate indication; a correct indication does NOT rescue a duplication. (a) List the patient's CURRENT meds and map each to its therapeutic class, INCLUDING combined/dual-class drugs — carvedilol/labetalol/propranolol/bisoprolol/metoprolol/nebivolol = beta-blocker; empagliflozin/dapagliflozin = SGLT2 inhibitor; ACE-I; ARB; sacubitril/valsartan (ARNI) = contains an ARB; spironolactone/eplerenone = MRA; amiloride = K-sparing (overlaps MRA); non-dihydropyridine CCB. (b) Map the keyed drug's class. (c) If the keyed drug's class is ALREADY PRESENT and concurrent use would ordinarily be DUPLICATE therapy (not a deliberate guideline combination), set valid=false, name both drugs + shared class, and state the fix is to SWITCH/ADJUST the existing agent, NOT add a second of the same class (e.g. dapagliflozin on empagliflozin; propranolol on carvedilol; a 2nd ACE-I/ARB incl. on an ARNI; a 2nd MRA). A genuine independent indication (e.g. propranolol for thyrotoxic tone) does NOT rescue it. (d) LEGITIMATE-COMBINATION GUARD: do NOT flag DIFFERENT classes used together on purpose — ACE-I/ARB + MRA + beta-blocker + SGLT2i are four distinct classes and are the correct HFrEF regimen; loop+thiazide sequential blockade; dual bronchodilators of different classes. Duplication = same class used redundantly, not different classes combined. (e) CONTRAINDICATION: valid=false if the keyed therapy is contraindicated by a stated drug/comorbidity (pioglitazone or non-DHP CCB in HFrEF; stopping metformin solely for eGFR >=30). Also flag wrong mechanism WORDING (e.g. hyperkalaemia peaked T via altered/accelerated repolarisation, not "delayed repolarisation").
STEP 2 — State the answer YOUR reconstruction lands on. BEFORE returning any verdict, CONSOLIDATE: re-confirm you actually applied BOTH the STEP 1b quantitative mechanism/trajectory check (a "pathognomonic" or MRI finding NEVER overrides contradictory numbers — e.g. a sodium rise of only ~2 mmol/L cannot cause osmotic demyelination) AND the STEP 1c duplication check. Do not return valid=true if either check reveals an inconsistency.
