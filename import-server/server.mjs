...
STEP 1c — REDUNDANT / CONTRAINDICATED THERAPY CHECK (V1.8, critical): read the patient's CURRENT medications and comorbidities in the stem, then check the keyed management against them. valid=false if the keyed answer is REDUNDANT (adds a drug of a class the patient is ALREADY taking — e.g. adding dapagliflozin when already on empagliflozin, a second ACE inhibitor/ARB, a second beta-blocker) or CONTRAINDICATED by a stated drug/comorbidity (e.g. a non-dihydropyridine calcium-channel blocker or pioglitazone in HFrEF, metformin stopped for eGFR ≥30 when guidelines only dose-reduce). Record the specific redundancy/contraindication in errors. Also sanity-check any stated electrophysiology/pharmacology mechanism wording against the facts (e.g. hyperkalaemia causes peaked T waves via ALTERED repolarisation, not "delayed repolarisation").
