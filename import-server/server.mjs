...
STEP 1c — REDUNDANT / CONTRAINDICATED THERAPY CHECK (V1.8.1, MANDATORY — do this explicitly, do not skip): (i) LIST every medication the patient is CURRENTLY taking as stated in the stem. (ii) Map each current medication to its DRUG CLASS (e.g. empagliflozin -> SGLT2 inhibitor; lisinopril -> ACE inhibitor; carvedilol -> beta-blocker; metformin -> biguanide). (iii) Identify the DRUG CLASS of the therapy in the keyed answer. (iv) If that keyed drug class MATCHES the class of any current medication, the keyed answer is REDUNDANT (a second drug of a class already prescribed - e.g. dapagliflozin added when already on empagliflozin = two SGLT2 inhibitors; a second ACE-inhibitor/ARB; a second beta-blocker). Set valid=false and name BOTH drugs and the shared class in errors. (v) Also set valid=false if the keyed therapy is CONTRAINDICATED by a stated drug or comorbidity (e.g. pioglitazone or a non-dihydropyridine calcium-channel blocker in HFrEF; stopping metformin solely for eGFR >=30 when guidelines only dose-reduce), naming the specific conflict. Do NOT pass an item just because the keyed drug stated benefit is individually true - if the patient already receives that class, adding another of the same class is still wrong. Also sanity-check any stated electrophysiology/pharmacology mechanism wording (e.g. hyperkalaemia causes peaked T waves via ALTERED/accelerated repolarisation, not "delayed repolarisation"); set valid=false and name the wording error.
