    ...
    .map(it => {
      ...
      return { stem:String(it.stem).trim(),
        ...
        rationales: (Array.isArray(it.rationales) ? it.rationales : []).slice(0,it.options.length).map(r=>String(r||"").trim()
            .replace(/^(correct|incorrect|wrong|right|true|false)\s*[:\-–—]\s*/i,"")               // drop verdict labels (Q3 keying bug)
            .replace(/^(the note states?( that)?|according to the (note|table|lecture)|as (per|stated in|noted in) the note|per the note)[:,]?\s*/i,"")   // drop "the note says…" (weak pedagogy)
            .trim()).map(r=> r ? r.charAt(0).toUpperCase()+r.slice(1) : r),
