...
## 🔵 QB-12 — MONITORING: raw→distinct dedup yield (not a bug yet)
First live build (Bronchiolitis) collapsed 15 raw → 8 distinct (7 near-dupes). 8 is at the very bottom of the ~10–15 target. **Not patched** — one sample isn't evidence of a systemic low yield, and changing the batch prompt right after verifying the pipeline end-to-end adds a variable. **Collect the signal first:** log raw / removed / final per build; if 7–9 recurs across topics, add a *prompt-level diversity mechanism* (distribute batches across skill × cognitive-level and distinct subtopics/clinical angles — fits the existing tag architecture) rather than weakening the de-dup. Decision: leave `/build-extra` untouched; revisit only with data.
