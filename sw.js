...
const CACHE = 'medbank-v226';   // v226: 02.3 replay contract — scheduler events now carry local_day, tz_offset and an algorithm@constant-fingerprint scheduler_version (still dark)   // v225: build 01 patch — qbank event now guards on result/item alignment (fixes shared response_ms) and carries the real topic_id   // v224: build 01 — durable learning_events ledger (dark, FEATURES.LEARNING_EVENTS=false)   // v223: dark mode (Light/Dark/Auto switch in the sidebar) + the account
