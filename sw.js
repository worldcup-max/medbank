...
const CACHE = 'medbank-v230';   // v230: viz3d.js asks "is this structure taught?" in ONE place (isTaught) instead of testing role === 'part' at four call sites. The spec declared only part|context while 92 structures across 25 scenes already used role:"primary" for the headline subject — the medulla, the ACA and MCA, every epithelium type — so all 92 were treated as scaffolding: kept out of the student's tappable list, given a context pin instead of a button, and CLIPPED AWAY in cross-sections. Three scenes carry no role:"part" at all and so had an empty parts list with nothing to tap: histology cell-junctions, histology simple-epithelia, neuroanatomy amygdala. Also: the anchor marker had transparent:true with no opacity, which is fully opaque in three — with depthTest:false that drew a solid ball in front of everything from every angle, hiding the bone it pointed at. BUMPED BECAUSE THE FETCH HANDLER IS CACHE-FIRST — viz3d.js changed again, and without this string changing every returning student keeps the copy they already have. Engine still frozen at v203.
// v229: the mesh loader's give-up clock now measures SILENCE, not elapsed time. Mesh resolution became a per-scene triangle budget (RENDER-STANDARD §5.5) and the muscle sheets went from ~1MB to 4-5MB; the old flat 12-second timeout then dropped whatever had not finished in 12s, which is never random — it is always the biggest file, and the biggest mesh in a scene is nearly always its subject. gross__thoracic-wall-diaphragm__intercostal-muscles was mounting with 32 of 35 structures, missing the external, internal and innermost intercostals: a bare rib cage in a scene about intercostal muscles. Measured on the live bucket, FMA9758 (3,931,984 bytes): the flat clock killed it at 12.5s holding 2,368,058 bytes after 60 progress events; a 15s stall timer re-armed on progress finished it in 15.7s. BUMPED BECAUSE THE FETCH HANDLER IS CACHE-FIRST — without this string changing, every returning student keeps the old viz3d.js and the fix reaches nobody.
// v228: procedural provider. viz3d.js registers a third provider beside bodyparts3d and svg: refs.procedural resolves "<model>#<part>" to a module under MODEL_BASE that builds the structure as a function of t. This is what lets a PROCESS be a scene — 32 embryology/histology scenes are written and were undrawable because there is no single mesh to scan for a tube that loops or a plate that folds. Engine still frozen at v203; the two existing providers are untouched. See viz-training/RENDER-STANDARD.md.
