...
const CACHE = 'medbank-v220';   // v220: a landmark now paints a coloured patch on the bone it sits on, the way an atlas marks an attachment, instead of only a dot with a leader line. The fetch handler is CACHE-FIRST, so viz3d.js changes are invisible to a device that already cached them until this string changes.
