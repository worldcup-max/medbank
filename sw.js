...
const CACHE = 'medbank-v218';   // v218: show-all now outranks the view's staging, and narration is pre-bought before it is needed (app.html + viz3d.js both changed). The fetch handler is CACHE-FIRST, so changes to either are invisible to a device that already cached them until this string changes. Bump it whenever a shipped asset changes behaviour.
