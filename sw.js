...
const CACHE = 'medbank-v219';   // v219: narration is pre-bought behind the model with a progress strip, Play waits for it, and the bone key light is brighter (app.html + viz3d.js both changed). The fetch handler is CACHE-FIRST, so changes to either are invisible to a device that already cached them until this string changes.
